...
@Autonomous(name ="Titan Red Left",group = "Titan")
public class TitanRedAutoLeft extends LinearOpMode{
  ...
  ElapsedTime  Runtime =new  ElapsedTime();
    ...
    public void runOpMode() {
        ...
        waitForStart();
            double timeout = getRuntime() + 5;
            while (opModeIsActive() && timeout > Runtime.seconds()) {
                if (cX < 220) {
                    path = "LEFT";
                } else if (cX >= 220 && cX <= 390) {
                    path = "CENTER";
        ...
        if(path.equals("LEFT")){
            robot.StrafeDist(1,12,0);
            robot.StrafeDist(-1,12,0);
        }
        if(path.equals("CENTER")){
            robot.driveStraight(1,12,0);
            robot.driveStraight(-1,12,0);
        }
        ...
        robot.driveStraight(1,24,90);
        robot.resetHeading();
        robot.driveStraight(1,96,0);
    }
    ...
    class BlueBlobDetectionPipeline extends OpenCvPipeline {
        ...
        private Mat preprocessFrame(Mat frame) {
            ...
            Scalar lowerBlue = new Scalar(200, 200, 200);
